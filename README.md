...
- 아카이브 및 최신화 로직: 데이터를 latest.json(최신 업데이트)과 news.json(전체 아카이브)으로 분리하여 프론트엔드 렌더링 성능과 데이터 파이프라인 효율을 극대화합니다.
...
- 데이터 저장소: 정적 JSON 파일 (news.json, latest.json)
...
[        새로운 데이터 병합 및 정렬        ]
           ...
           v
+--------------------------------------+
|        public/data/news.json         | (업데이트된 전체 아카이브)
+--------------------------------------+
           |
           | 5. 최신 소식 분리 추출
           v
