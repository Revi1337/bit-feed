...
import { GEMINI_RATE_LIMIT_DELAY_MS, LATEST_JSON_MAX_ITEMS } from '../config/constants.mjs';
...
async function fillMissing() {
  ...
  if (missingItems.length > 0) {
    ...
    const topItems = data.slice(0, LATEST_JSON_MAX_ITEMS);
    await fs.writeFile(latestPath, JSON.stringify(topItems, null, 2), 'utf-8');
